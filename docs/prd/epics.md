...
- **Progress (2026-07-26):**
  ...
  - [ ] Standardisasi `ok`/`success` — kontrak target + rencana migrasi: `docs/api/gas-contracts.md` §8.
    ...
    - [ ] Fase B backend — **kode siap di repo (2026-07-27), tinggal deploy clasp**: normalisasi `jsonAbsensi_` GAS B; envelope GET sheet generik GAS A (0 konsumen frontend, aman); reset page dual-check; `import-data-obat.gs` disamakan + ditandai legacy/orphan. Remote HEAD ≡ lokal pra-patch (verified). Deploy = `clasp push` + `create-version` + `update-deployment` ke ID live kedua project.
