...
var SlipGajiBulanan = (function() {
  var CONFIG = {
    ...
    // Seluruh tampilan template slip sampai total gaji bersih.
    exportRange: 'C1:K32',
    folderName: 'slip_gaji_pdf',
    logSheetName: 'log_slip_gaji',
    deliveryLogSheetName: 'log_kirim_slip_gaji',
    timezone: 'Asia/Jakarta',
    sendHour: 8,
    // Beri waktu formula/dropdown template selesai dihitung sebelum ekspor PDF.
    renderDelayMs: 3000,
    ...
    try {
      ...
      nips.forEach(function(nip) {
        ...
        try {
          ...
          if (typeof writePayrollHistoryEntry_ === 'function') {
            writePayrollHistoryEntry_(spreadsheet, item, period, {
  ...
  function createOneSlipFile(spreadsheet, templateSheet, folder, contacts, nip, period, config) {
    var payrollSheet = typeof getPayrollSheet_ === 'function'
      ? getPayrollSheet_(spreadsheet)
      : spreadsheet.getSheetByName(config.contactSheetName);
    var employees = payrollSheet && typeof readPayrollEmployees_ === 'function'
      ? readPayrollEmployees_(payrollSheet)
      : [];
    var contact = findContact(contacts, nip, '');
    var employee = typeof findPayrollEmployee_ === 'function'
      ? findPayrollEmployee_(employees, nip, contact.name || '')
      : null;
    var exportSheet = null;

    if (!employee) {
      throw new Error('Data gaji untuk NIP ' + nip + ' tidak ditemukan di sheet data_karyawan.');
    }

    period = normalizeAutomaticPayrollPeriod_(period);
    exportSheet = templateSheet.copyTo(spreadsheet);

    try {
      exportSheet.setName('_Slip_Gaji_Auto_' + Utilities.formatDate(new Date(), config.timezone || 'Asia/Jakarta', 'yyyyMMdd_HHmmss'));

      var summary = typeof calculatePayrollAttendanceSummary_ === 'function'
        ? calculatePayrollAttendanceSummary_(spreadsheet, employee, period)
        : {};
      var salary = typeof calculatePayrollSalaryTotals_ === 'function'
        ? calculatePayrollSalaryTotals_(employee, summary)
        : {};
      var name = String(employee.name || contact.name || nip || '').trim();

      if (!name) {
        throw new Error('Nama karyawan untuk NIP ' + nip + ' kosong di sheet data_karyawan.');
      }

      if (typeof writePayrollTemplateContext_ !== 'function') {
        throw new Error('Fungsi writePayrollTemplateContext_ belum tersedia. Pastikan Kode.js terbaru sudah disalin ke project Apps Script yang sama.');
      }

      writePayrollTemplateContext_(exportSheet, employee, period, summary, salary);
      SpreadsheetApp.flush();
      Utilities.sleep(Math.max(350, Math.min(Number(config.renderDelayMs || 0) || 350, 5000)));

      if (!hasSlipContent(exportSheet, config)) {
        throw new Error('Template slip belum merender data untuk NIP ' + nip + '. Periksa template Slip_Gaji dan helper payroll terbaru.');
      }

      var pdfFile = typeof exportPayrollSlipPdf_ === 'function'
        ? exportPayrollSlipPdf_(spreadsheet, exportSheet, folder, employee, period)
        : exportPdf(spreadsheet, exportSheet, folder, nip, name, period, config);
      ...
      if (exportSheet) spreadsheet.deleteSheet(exportSheet);
    }
  }

  function normalizeAutomaticPayrollPeriod_(period) {
    if (period && period.start && period.end && period.label && period.fileStamp) {
      return period;
    }

    if (typeof buildPayrollPeriod_ === 'function') {
      var stampMatch = String(period && period.fileStamp || '').match(/^(\d{4})-(\d{2})$/);
      if (stampMatch) {
        return buildPayrollPeriod_(stampMatch[2], stampMatch[1]);
      }

      if (period && period.monthNumber && period.year) {
        return buildPayrollPeriod_(period.monthNumber, period.year);
      }
    }
  ...
  function writePayrollHistoryEntry_(spreadsheet, slip, period, payload) {
    if (!slip || typeof writePayrollLog_ !== 'function') return 0;
    return writePayrollLog_(spreadsheet, slip.employee, period, slip.summary, slip.salary, slip.file, payload || {});
  }

  function hasSlipContent(templateSheet, config) {
    var values = templateSheet.getRange(config.exportRange).getDisplayValues();
    var meaningfulCells = 0;

    values.forEach(function(row) {
      row.forEach(function(value) {
        var text = String(value || '').trim();
        if (text && !/^(rp\.?|[-–—])$/i.test(text)) meaningfulCells += 1;
      });
    });

    return meaningfulCells >= 5;
  }

  function snapshotTemplateCells(templateSheet, cells) {
    return uniqueList(cells || []).map(function(a1) {
      var range = templateSheet.getRange(a1);
      return {
        range: range,
  ...
  function restoreTemplateCells(snapshot) {
    (snapshot || []).forEach(function(item) {
      if (!item || !item.range) return;
      if (item.formula) item.range.setFormula(item.formula);
      else item.range.setValue(item.value);
    });
    SpreadsheetApp.flush();
  }
  ...
  function readConfig() {
    ...
    return {
      ...
      deliveryLogSheetName: String(props.getProperty('SLIP_DELIVERY_LOG_SHEET_NAME') || CONFIG.deliveryLogSheetName).trim(),
  ...
  function writeLog(spreadsheet, config, item) {
    var sheetName = resolveDeliveryLogSheetName(config);
  ...
  function resolveDeliveryLogSheetName(config) {
    var configured = String(config.deliveryLogSheetName || (config.logSheetName + '_delivery') || 'log_kirim_slip_gaji').trim();
    var payrollLogName = String(config.logSheetName || 'log_slip_gaji').trim();

    if (!configured) return 'log_kirim_slip_gaji';
    return normalizeKey(configured) === normalizeKey(payrollLogName) ? 'log_kirim_slip_gaji' : configured;
  }
  ...
  function previousMonthPeriod() {
    ...
    if (typeof buildPayrollPeriod_ === 'function') {
      return buildPayrollPeriod_(start.getMonth() + 1, start.getFullYear());
    }

    return {
      label: MONTHS_ID[start.getMonth()] + ' ' + start.getFullYear(),
      fileStamp: Utilities.formatDate(start, CONFIG.timezone, 'yyyy-MM'),
      month: Utilities.formatDate(start, CONFIG.timezone, 'MM'),
      monthNumber: Number(Utilities.formatDate(start, CONFIG.timezone, 'M')),
      year: Utilities.formatDate(start, CONFIG.timezone, 'yyyy'),
      start: Utilities.formatDate(start, CONFIG.timezone, 'yyyy-MM-01'),
      end: Utilities.formatDate(new Date(start.getFullYear(), start.getMonth() + 1, 0), CONFIG.timezone, 'yyyy-MM-dd')
    };
  }
