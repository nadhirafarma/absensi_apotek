(function () {
  ...
  const state = { records: [], groups: [], loading: false, error: "", salarySlips: [], salaryPromise: null, salaryError: "", salaryLoadedAt: 0, employeeTab: "dashboard", monitoringTab: "dashboard", employeeFilter: { date: "", status: "" }, monitoringFilter: { date: "", status: "", search: "", salarySearch: "" }, loadedAt: 0 };
  const icons = {
  ...
  function normalizeSalarySlip(row) { return { id: String(row.id || row.fileId || row.rowNumber || ""), rowNumber: Number(row.rowNumber || 0), issuedAt: String(row.issuedAt || ""), period: String(row.period || row.periode || "-"), nip: String(row.nip || row.NIP || "-"), name: String(row.name || row.nama || "-"), netSalary: Number(row.netSalary || row.gajiBersih || 0), fileName: String(row.fileName || "Slip Gaji.pdf"), fileUrl: safePdfUrl(row.fileUrl || row.printUrl) }; }
  ...
  async function loadSalarySlips(force) {
    ...
    if (!force && Date.now() - state.salaryLoadedAt < 120000) return;
    state.salaryError = "";
    state.salaryPromise = (async () => {
      try {
        ...
        state.salarySlips = data.history.map(normalizeSalarySlip);
        state.salaryTotal = Number(data.total || 0);
  ...
  function salarySlipTable(items, monitoring) {
    const rows = items.length ? items.map((item) => `<tr>${monitoring ? `<td><strong>${esc(item.name)}</strong></td><td>${esc(item.nip)}</td>` : ""}<td>${esc(item.period)}</td><td>${esc(salaryDate(item.issuedAt))}</td><td><strong>${esc(rupiah(item.netSalary))}</strong></td><td>${item.fileUrl ? `<a class="ess-tool-button is-primary" href="${esc(item.fileUrl)}" target="_blank" rel="noopener noreferrer">Buka PDF</a>` : badge("PDF tidak tersedia")}</td></tr>`).join("") : `<tr><td colspan="${monitoring ? 6 : 4}">Belum ada slip gaji PDF yang diterbitkan.</td></tr>`;
    return `<div class="ess-table-wrap"><table class="ess-table ess-salary-table"><thead><tr>${monitoring ? "<th>Karyawan</th><th>NIP</th>" : ""}<th>Periode</th><th>Diterbitkan</th><th>Gaji Bersih</th><th>PDF</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }
  ...
  function renderMonitoringSalary() { const c = document.getElementById("monitoringContent"); if (!c) return; const slips = getMonitoringSalarySlips(); c.innerHTML = `<article class="ess-panel"><div class="ess-panel-head"><div><h4>Monitoring Slip Gaji</h4><p>${esc(state.salarySlips.length)} slip PDF dari seluruh karyawan.</p></div><button class="ess-tool-button" type="button" data-salary-refresh="monitoring">Sinkron ulang</button></div><div class="ess-table-tools"><input class="ess-control" id="monitorSalarySearch" type="search" value="${esc(state.monitoringFilter.salarySearch)}" placeholder="Cari nama, NIP, atau periode..."></div>${salaryErrorLine()}${salarySlipTable(slips, true)}</article>`; }
