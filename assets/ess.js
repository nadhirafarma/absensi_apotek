(function () {
  ...
  const initialSalaryFilter = { month: monthKey().slice(5), year: monthKey().slice(0, 4), page: 1, limit: 10 };
  const state = { records: [], groups: [], loading: false, error: "", salarySlips: [], salaryPromise: null, salaryError: "", salaryLoadedAt: 0, salaryTotal: 0, salaryHasMore: false, salaryFilter: { ...initialSalaryFilter }, employeeTab: "dashboard", monitoringTab: "dashboard", employeeFilter: { date: "", status: "" }, monitoringFilter: { date: "", status: "", search: "", salarySearch: "" }, loadedAt: 0 };
  const icons = {
  ...
  function safePdfUrl(value) { try { const url = new URL(String(value || "")); return url.protocol === "https:" ? url.href : ""; } catch (_) { return ""; } }
  function salaryDate(value) { const d = new Date(value); return Number.isNaN(d.getTime()) ? String(value || "-") : new Intl.DateTimeFormat("id-ID", { day: "2-digit", month: "short", year: "numeric" }).format(d); }
  function normalizeSalarySlip(row) { const rawSalary = row.netSalary ?? row.gajiBersih; return { id: String(row.id || row.fileId || row.rowNumber || ""), rowNumber: Number(row.rowNumber || 0), issuedAt: String(row.issuedAt || ""), period: String(row.period || row.periode || "-"), nip: String(row.nip || row.NIP || "-"), name: String(row.name || row.nama || "-"), netSalary: Number(rawSalary || 0), netSalaryAvailable: row.netSalaryAvailable !== false && String(rawSalary ?? "").trim() !== "", fileId: String(row.fileId || ""), fileName: String(row.fileName || "Slip Gaji.pdf"), fileUrl: safePdfUrl(row.fileUrl || row.printUrl) }; }
  function dedupeSalarySlips(rows) { const seen = new Set(); return rows.map(normalizeSalarySlip).filter((item) => { const key = item.fileId || item.fileUrl || `${item.nip}|${item.period}|${item.fileName}|${item.issuedAt}`; if (!key || seen.has(key)) return false; seen.add(key); return true; }).sort((a, b) => new Date(b.issuedAt || 0) - new Date(a.issuedAt || 0) || b.rowNumber - a.rowNumber); }
  ...
  async function load(force) {
    if (state.loading || (!force && Date.now() - state.loadedAt < 600000)) return;
  ...
  async function loadSalarySlips(force) {
    if (state.salaryPromise) return state.salaryPromise;
    if (!force && Date.now() - state.salaryLoadedAt < 600000) return;
    state.salaryError = "";
    state.salaryPromise = (async () => {
      try {
        const s = session();
        const url = new URL(API_URL);
        url.searchParams.set("action", "listSalarySlipHistory");
        url.searchParams.set("sessionToken", s.sessionToken || "");
        url.searchParams.set("name", s.name || s.username || "");
        const filter = state.salaryFilter;
        url.searchParams.set("month", filter.month);
        url.searchParams.set("year", filter.year);
        url.searchParams.set("page", String(filter.page));
        url.searchParams.set("limit", String(filter.limit));
        const res = await fetch(url.toString(), { cache: "no-store" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        if (!isApiOk(data) || !Array.isArray(data.history)) throw new Error(data?.message || "Histori slip gaji gagal dimuat.");
        const serverPaged = Object.prototype.hasOwnProperty.call(data, "hasMore") || Object.prototype.hasOwnProperty.call(data, "filteredTotal");
        const remote = dedupeSalarySlips(data.history);
        state.salarySlips = serverPaged ? remote : remote.slice((state.salaryFilter.page - 1) * state.salaryFilter.limit, state.salaryFilter.page * state.salaryFilter.limit);
        state.salaryTotal = Number(data.filteredTotal ?? data.total ?? remote.length);
        state.salaryHasMore = data.hasMore === true || (!serverPaged && state.salaryFilter.page * state.salaryFilter.limit < remote.length);
        state.salaryLoadedAt = Date.now();
      } catch (error) { state.salaryError = error.message || "Histori slip gaji belum tersedia."; }
      finally { state.salaryPromise = null; }
    })();
    return state.salaryPromise;
  }
  ...
  function salarySlipTable(items, monitoring) {
    // BUG 2 & 5 FIX: show "Nama - Periode" format, one line with ellipsis
    const rows = items.length ? items.map((item) => `<tr><td class="ess-salary-period" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:320px"><strong>${esc(item.name && item.name !== "-" ? item.name + " - " + item.period : item.period)}</strong></td><td>${item.fileUrl ? `<a class="ess-tool-button is-primary" href="${esc(item.fileUrl)}" target="_blank" rel="noopener noreferrer">Buka PDF</a>` : badge("PDF tidak tersedia")}</td></tr>`).join("") : `<tr><td colspan="2">Belum ada slip gaji PDF yang diterbitkan.</td></tr>`;
    return `<div class="ess-table-wrap"><table class="ess-table ess-salary-table"><thead><tr><th>Nama File PDF</th><th>Aksi</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }
  function salaryErrorLine() { return state.salaryError ? `<div class="ess-error">${esc(state.salaryError)}</div>` : ""; }
  ...
  function getMonitoringSalarySlips() { const search = norm(state.monitoringFilter.salarySearch); return state.salarySlips.filter((item) => !search || norm(`${item.name} ${item.nip} ${item.period}`).includes(search)); }
  function salaryMonthOptions(value) { return Array.from({ length: 12 }, (_, index) => { const month = String(index + 1).padStart(2, "0"); const label = new Intl.DateTimeFormat("id-ID", { month: "long" }).format(new Date(2026, index, 1)); return `<option value="${month}"${value === month ? " selected" : ""}>${esc(label)}</option>`; }).join(""); }
  function salaryYearOptions(value) { const currentYear = Number(monthKey().slice(0, 4)) || new Date().getFullYear(); return Array.from({ length: 5 }, (_, index) => { const year = currentYear - 2 + index; return `<option value="${year}"${String(year) === String(value) ? " selected" : ""}>${year}</option>`; }).join(""); }
  function salaryFilterTools(monitoring) { const f = state.salaryFilter; return `<div class="ess-table-tools ess-salary-filters"><label class="ess-salary-field"><span>Bulan</span><select class="ess-control" id="essSalaryMonth">${salaryMonthOptions(f.month)}</select></label><label class="ess-salary-field"><span>Tahun</span><select class="ess-control" id="essSalaryYear">${salaryYearOptions(f.year)}</select></label>${monitoring ? `<label class="ess-salary-field ess-salary-search"><span>Cari</span><input class="ess-control" id="monitorSalarySearch" type="search" value="${esc(state.monitoringFilter.salarySearch)}" placeholder="Nama, NIP, atau periode..."></label>` : ""}<div class="ess-salary-actions"><button class="ess-tool-button is-primary ess-salary-apply" type="button" data-salary-apply>Terapkan Filter</button><button class="ess-tool-button" type="button" data-salary-reset>Reset</button></div></div>`; }
  function salaryPagination() { const totalPages = Math.max(1, Math.ceil(state.salaryTotal / state.salaryFilter.limit)); if (totalPages <= 1) return ""; const page = state.salaryFilter.page; return `<div class="ess-table-tools"><span>${esc(`${state.salaryTotal} hasil · halaman ${page}/${totalPages}`)}</span><button class="ess-tool-button" type="button" data-salary-page="${page - 1}"${page <= 1 ? " disabled" : ""}>Sebelumnya</button><button class="ess-tool-button" type="button" data-salary-page="${page + 1}"${page >= totalPages && !state.salaryHasMore ? " disabled" : ""}>Berikutnya</button></div>`; }
  function applySalaryFilterFromControls() { state.salaryFilter = { ...state.salaryFilter, month: document.getElementById("essSalaryMonth")?.value || monthKey().slice(5), year: document.getElementById("essSalaryYear")?.value || monthKey().slice(0, 4), page: 1 }; return true; }
  function resetSalaryFilter() { state.salaryFilter = { ...initialSalaryFilter }; state.monitoringFilter.salarySearch = ""; }
  ...
  function renderEmployeeSalary() { const c = document.getElementById("essContent"); if (!c) return; if (isAdmin()) { c.innerHTML = `<article class="ess-panel">${empty("salary", "Gunakan Monitoring Slip Gaji", "Akun owner/admin melihat seluruh histori melalui menu Monitoring Presensi.")}</article>`; return; } c.innerHTML = `<article class="ess-panel"><div class="ess-panel-head"><div><h4>Slip Gaji Saya</h4><p>Slip gaji PDF yang diterbitkan owner/admin untuk akun Anda.</p></div><button class="ess-tool-button" type="button" data-salary-refresh="employee">Sinkron ulang</button></div>${salaryFilterTools(false)}${salaryErrorLine()}${salarySlipTable(state.salarySlips, false)}${salaryPagination()}</article>`; }
  ...
  function renderMonitoringSalary() { const c = document.getElementById("monitoringContent"); if (!c) return; const slips = getMonitoringSalarySlips(); c.innerHTML = `<article class="ess-panel"><div class="ess-panel-head"><div><h4>Monitoring Slip Gaji</h4><p>${esc(slips.length)} dari ${esc(state.salaryTotal)} hasil.</p></div><button class="ess-tool-button" type="button" data-salary-refresh="monitoring">Sinkron ulang</button></div>${salaryFilterTools(true)}${salaryErrorLine()}${salarySlipTable(slips, true)}${salaryPagination()}</article>`; }
  ...
  async function switchEmployee(name) { state.employeeTab = name; document.querySelectorAll("[data-ess-tab]").forEach((b) => b.classList.toggle("is-active", b.dataset.essTab === name)); if (name === "salary" && !isAdmin()) { const c = document.getElementById("essContent"); if (c) c.innerHTML = `<article class="ess-panel">${empty("salary", "Memuat slip gaji...", "Menyinkronkan histori PDF untuk akun Anda.")}</article>`; await loadSalarySlips(true); if (state.employeeTab !== name) return; } if (name === "dashboard") renderESSDashboard(); else if (name === "attendance") renderEmployeeAttendance(); else if (name === "salary") renderEmployeeSalary(); else renderEmployeeModule(name); }
  async function switchMonitoring(name) { if (!isAdmin()) return; state.monitoringTab = name; document.querySelectorAll("[data-monitoring-tab]").forEach((b) => b.classList.toggle("is-active", b.dataset.monitoringTab === name)); if (name === "salary") { const c = document.getElementById("monitoringContent"); if (c) c.innerHTML = `<article class="ess-panel">${empty("salary", "Memuat histori slip...", "Menyinkronkan PDF seluruh karyawan.")}</article>`; await loadSalarySlips(true); if (state.monitoringTab !== name) return; } if (name === "dashboard") renderMonitoringDashboard(); else if (name === "attendance") renderMonitoringAttendance(); else if (name === "salary") renderMonitoringSalary(); else renderMonitoringModule(name); }
  ...
  function bind() { document.addEventListener("click", async (event) => { const e = event.target.closest("[data-ess-tab]"); if (e) await switchEmployee(e.dataset.essTab); const m = event.target.closest("[data-monitoring-tab]"); if (m) await switchMonitoring(m.dataset.monitoringTab); if (event.target.closest("[data-ess-export]")) exportCsv(false); if (event.target.closest("[data-monitoring-export]")) exportCsv(true); if (event.target.closest("[data-open-presensi]")) document.querySelector('[data-view-target="presensi"]')?.click(); if (event.target.closest("[data-salary-apply]")) { if (applySalaryFilterFromControls()) { await loadSalarySlips(true); isAdmin() ? renderMonitoringSalary() : renderEmployeeSalary(); } else { isAdmin() ? renderMonitoringSalary() : renderEmployeeSalary(); } } else if (event.target.closest("[data-salary-reset]")) { resetSalaryFilter(); await loadSalarySlips(true); isAdmin() ? renderMonitoringSalary() : renderEmployeeSalary(); } else { const pageButton = event.target.closest("[data-salary-page]"); if (pageButton && !pageButton.disabled) { state.salaryFilter.page = Number(pageButton.dataset.salaryPage || 1); await loadSalarySlips(true); isAdmin() ? renderMonitoringSalary() : renderEmployeeSalary(); } else if (event.target.closest('[data-salary-refresh="monitoring"]')) { await loadSalarySlips(true); renderMonitoringSalary(); } else if (event.target.closest('[data-salary-refresh="employee"]')) { await loadSalarySlips(true); renderEmployeeSalary(); } } }); document.addEventListener("input", (event) => { if (event.target.id === "essSearchDate") { state.employeeFilter.date = event.target.value; renderEmployeeAttendance(); } if (event.target.id === "monitorSearch") { state.monitoringFilter.search = event.target.value; renderMonitoringAttendance(); } if (event.target.id === "monitorSalarySearch") { state.monitoringFilter.salarySearch = event.target.value; renderMonitoringSalary(); } }); document.addEventListener("change", (event) => { if (event.target.id === "essStatusFilter") { state.employeeFilter.status = event.target.value; renderEmployeeAttendance(); } if (event.target.id === "monitorDate") { state.monitoringFilter.date = event.target.value; renderMonitoringAttendance(); } if (event.target.id === "monitorStatus") { state.monitoringFilter.status = event.target.value; renderMonitoringAttendance(); } }); document.querySelector("[data-ess-refresh]")?.addEventListener("click", async () => { if (state.employeeTab === "salary") { await loadSalarySlips(true); renderEmployeeSalary(); return; } await load(true); await switchEmployee(state.employeeTab); }); document.querySelector("[data-monitoring-refresh]")?.addEventListener("click", async () => { if (state.monitoringTab === "salary") { await loadSalarySlips(true); renderMonitoringSalary(); return; } await load(true); await switchMonitoring(state.monitoringTab); }); }
